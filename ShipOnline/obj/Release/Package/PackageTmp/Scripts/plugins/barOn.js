        ...
        BarOn: function (S, opt) { /* Scrollbars */
            ...
            Bo=(S.to = (S = $(S))).BarOn = {
            	...
            	drawScroll : function (e) {
            		...
            		if (!(SS = S) || !(Sto = S.tb) || !Sto[0]) { Bo.buildBar(); return };
            		...
            		do {
            			...
            			if (ox && Sto.data('overflow-x') in Bo.doList) {
            				...
            				if ((sw=Sto[0].scrollWidth)-(iw=bB.width())>2) {
            				    sbB.left = Bo.round(t = Sto[0].scrollLeft * (1 + (a = iw / sw))) + 'px';
            				    sbB.bottom = Bo.round(-Sto[0].scrollTop + ((sh = Sto.height() + Sto.position().top - Sto.offsetParent().height()) > 0 ? sh : 0)) + 'px';
            				    sbB.right = Bo.round((sw - iw) * a - t + 1) + 'px';
            					sbB.display = 'block';
            					ox=false;
            				};
            			};
            			if (oy && Sto.data('overflow-y') in Bo.doList) {
	            			...
	            			if ((sh=Sto[0].scrollHeight)-(ih=rB.height())>2) {
	            			    srB.top = Bo.round(t = Sto[0].scrollTop * (1 + (a = ih / sh))) + 'px';
	            			    srB.right = Bo.round(-Sto[0].scrollLeft + ((sw = Sto.width() + Sto.position().left - Sto.offsetParent().width()) > 0 ? sw : 0)) + 'px';
	            			    srB.bottom = Bo.round((sh - ih) * a - t + 1) + 'px';
	            				srB.display = 'block';
	            				oy=false;
	            			};
            			};
            	...
            	buildBar: function () {
            	    var sw = $('.dragOn')[0].scrollWidth,
                        t = $('.dragOn')[0].scrollLeft
            	        iw = $('.dragOn').width(),
                        a = iw / sw;
            	    if (sw - iw > 2) {
            	        $('.bBarOn').css('right', Bo.round((sw - iw) * a - t + 1) + 'px');
            	    };
            	}
            };
