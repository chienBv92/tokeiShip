...
$(function () {
    ...
    $.extend({
        DragOn: function (S, opt) { /* Scroll mechanics */
            var def = {
                exclusion: { 'th': '', 'input': '', 'textarea': '', 'select': '', 'object': '', 'iframe': '', 'details': '', 'id': '#gmap,#map-canvas,.button,[data-overflow=no-dragon],a,.btn > :not(img),.text-overflow,.radio-button,.btn' },
            	cursor : 'all-scroll',
